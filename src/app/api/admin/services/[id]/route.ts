...
import {
  deleteLocalServiceByIdOrFallbackSlug,
...
export async function GET(_request: NextRequest, { params }: RouteParams) {
  ...
  const service = !prisma
    ? await getLocalServiceByIdOrFallbackSlug(id)
    : await resolveServiceByIdOrFallbackSlug(id)
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    if (!prisma) {
      const service = await upsertLocalServiceByIdOrFallbackSlug(id, payload)
      return NextResponse.json(service)
    }
...
export async function DELETE(_request: NextRequest, { params }: RouteParams) {
  ...
  try {
    if (!prisma) {
      const removed = await deleteLocalServiceByIdOrFallbackSlug(id)
      if (!removed) {
        return NextResponse.json({ error: 'Service not found.' }, { status: 404 })
      }
